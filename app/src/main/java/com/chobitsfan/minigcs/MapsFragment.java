...
public class MapsFragment extends Fragment implements OnMapReadyCallback {
        ...
        mViewModel.getGlobalPos().observe(getViewLifecycleOwner(), pos->{
            if (googleMap != null) {
                ...
                if (lat > 0 &&  lon > 0) {
                    ...
                    if (droneMarker == null) {
                        droneMarker = googleMap.addMarker(new MarkerOptions().anchor(0.5f,0.5f).flat(true).rotation(pos.hdg()*0.01f).position(droneLatlng).icon(BitmapDescriptorFactory.fromResource(R.drawable.drone_arrow)));
                        googleMap.moveCamera(CameraUpdateFactory.newLatLngZoom(droneLatlng, 18));
                    } else {
                        droneMarker.setPosition(droneLatlng);
                        droneMarker.setRotation(pos.hdg()*0.01f);
    ...
    public void onMapReady(@NonNull GoogleMap googleMap) {
        ...
        if (dronePos != null) {
            ...
            if (lat > 0 && lon > 0) {
                LatLng droneLatlng = new LatLng(lat * 1e-7, lon * 1e-7);
                droneMarker = googleMap.addMarker(new MarkerOptions().anchor(0.5f,0.5f).flat(true).rotation(dronePos.hdg()*0.01f).position(droneLatlng).icon(BitmapDescriptorFactory.fromResource(R.drawable.drone_arrow)));
                googleMap.moveCamera(CameraUpdateFactory.newLatLngZoom(droneLatlng, 18));
            }
        }
